...
let _isHasPermission = (type, permissionID, params) => {
  switch (type) {
    ...
    case 2:
      {
        ...
        if (!versionPermission[permissionID] || parseInt(versionPermission[permissionID].useFlag) === 0) {
          return {
            status: 0,
            comClass: ''
          };
    ...
    case 3:
      {
        const versionPermission = state.permissions.versionPermission;
        if (versionPermission[permissionID] && typeof versionPermission[permissionID].numLimit !== 'undefined') {
    ...
    case 4:
      {
        ...
        if (state.companyInfo.version === 4) { //免费版本
          const module = perssionList[permissionID];
          if (module && module.useFlag === 1 && module.type === 53) { //useFlg 1 type 53不可以跳转，其他可以跳转
            return false;
          }
        }
        return true;

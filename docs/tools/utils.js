...
const _requestYB = (req, url, data, callback) => {
  ...
  postdata.data.platform = { wechat: 0, other: 2, ios: 3, android: 4 }[
    _getPlatform(req.headers['user-agent'])
  ];
...
const _decryptData = (toDecrypt, callBack) => {
  ...
  zlib.gunzip(buffer, (err, decoded) => {
    ...
    try {
      const resData = Object.assign({}, toDecrypt, {
        data: JSON.parse(parseStr),
      });
